...
以「臺北市運動科學中心」為角色情境的 Athletic Management System 教學原型。v0.4 批次整理 113 年全中運官方決賽成績報告表中，臺北市高中組羽球、田徑、游泳的 157 筆成績紀錄（17 所學校），並保留選手歷程、檢測、處方與教練協作的資料架構。
...
- 選手管理：運動、學校與關鍵字交叉篩選；每筆成績可回到官方來源。
- 訓練課表：不再限制五天，可持續往下新增並保存於目前瀏覽器。
- 課表資料庫：羽球、游泳及田徑分項模組；田徑包含短距離、中距離、長距離、跳部、擲部及全能。
- 週期管理：可選整隊／個人、單位、運動及男／女子組，並可調整每日負荷。
- 機能重建：個人選擇、現況／病理紀錄、功能初篩、ADL、低中高強度、回隊評估及團隊決策。

訓練模組的知識架構參考 ACSM 青少年阻力訓練安全原則、NSCA《Essentials of Strength Training and Conditioning》第五版章節架構，以及 return-to-sport continuum／PAASS 概念。這些內容是處方設計輔助，不取代醫師、物理治療師、運動防護員或合格肌力與體能專業人員的判斷。

## 公開資料匯入

```bash
python3 scripts/import_113_results.py athletics.html swimming.html badminton.html src/athletes.generated.ts
```

產出的 `src/athletes.generated.ts` 是歷史決賽紀錄，不是目前在學名冊。團體與接力項目依官方表格保留為組合紀錄，避免自行拆分或推定個人身分。
